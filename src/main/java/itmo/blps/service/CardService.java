...
import itmo.blps.dto.request.EventRequest;
import itmo.blps.dto.response.CardResponse;
import itmo.blps.dto.response.EventResponse;
...
public class CardService {
    ...
    private final ConcurrentHashMap<String, CompletableFuture<EventResponse>> pendingRequests = new ConcurrentHashMap<>();
    ...
    public CardResponse topUpBalance(BalanceRequest balanceRequest, String username) {
        try {
            ...
            CompletableFuture<EventResponse> responseFuture = new CompletableFuture<>();
            pendingRequests.put(correlationId, responseFuture);

            Map<String, Object> messageData = Map.of(
                    "correlationId", correlationId,
                    "number", balanceRequest.getNumber(),
                    "money", balanceRequest.getMoney()
            );
            jmsTemplate.convertAndSend(orderPaymentQueue, messageData, message -> {
//                message.setJMSCorrelationID(correlationId);
                message.setStringProperty("_type", "EventRequest");
                message.setStringProperty("Content-Type", "application/json");
                return message;
            });

//            jmsTemplate.convertAndSend(orderPaymentQueue, new EventRequest(correlationId, balanceRequest.getNumber(), balanceRequest.getMoney()), message -> {
//                message.setJMSCorrelationID(correlationId);
//                message.setStringProperty("_type", "EventRequest");
//                message.setStringProperty("Content-Type", "application/json");
//                return message;
//            });
//            jmsTemplate.convertAndSend(orderPaymentQueue, new EventRequest(correlationId, balanceRequest.getNumber(), balanceRequest.getMoney()));

            EventResponse eventResponse = responseFuture.get(10, TimeUnit.SECONDS);
